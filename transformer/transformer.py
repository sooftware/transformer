...
from transformer.mask import subsequent_masking, pad_masking
from transformer.modules import Linear
from transformer.embeddings import Embedding, PositionalEncoding
from transformer.layers import TransformerEncoderLayer, TransformerDecoderLayer
...
class Transformer(nn.Module):
    ...
    Args:
        ...
        poswise_ffnet (str): if poswise_ffnet is 'ff', position-wise feed forware network to be a feed forward,
    ...
    def __init__(self, num_classes: int, pad_id: int, num_input_embeddings: int, num_output_embeddings: int,
                 ...
                 dropout_p: float = 0.3, poswise_ffnet: str = 'ff'):
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor):
        ...
        inputs_mask = pad_masking(inputs, input_length)
        memory_mask = pad_masking(inputs, target_length)
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, num_embeddings: int, d_model: int = 512, d_ff: int = 2048,
                 num_layers: int = 6, num_heads: int = 8, poswise_ffnet: str = 'ff',
        ...
        self.layers = nn.ModuleList(
            [TransformerEncoderLayer(d_model, num_heads, d_ff, dropout_p, poswise_ffnet) for _ in range(num_layers)]
        )
