...
class SupervisedTrainer(object):
    ...
    Args:
        ...
        loss (transformer.loss.loss.Loss, optional): loss for training, (default: seq2seq.loss.NLLLoss)
    ...
    def _train_batch(self, inputs, targets, model):
        criterion = nn.NLLLoss()
        # Forward propagation
        output, encoder_self_attns, decoder_self_attns, decoder_encoder_attns = model(inputs, targets)
        # Get loss
    ...
    def _train_epoches(self, data, model, n_epochs, start_epoch, start_step, dev_data=None):
        ...
        for epoch in range(start_epoch, n_epochs + 1):
            ...
            for batch in batch_generator:
                ...
                loss = self._train_batch(inputs, targets, model)
    ...
    def train(self, model, data, num_epochs=5, resume=False, dev_data=None, optimizer=None):
        """
        Run training for a given model.

        Args:
            model (transformer.models): model to run training on, if `resume=True`, it would be
               overwritten by the model loaded from the latest checkpoint.
            data (transformer.dataset.dataset.Dataset): dataset object to train on
            num_epochs (int, optional): number of epochs to run (default 5)
            resume(bool, optional): resume training with the latest checkpoint, (default False)
            dev_data (transformer.dataset.dataset.Dataset, optional): dev Dataset (default None)
            optimizer (transformer.optim.Optimizer, optional): optimizer for training
